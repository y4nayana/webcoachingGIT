...
<table class="table" id="datakomen">
  ...
  <tbody>
    <?php foreach ($coaching as $key => $schedule) { ?>
      <tr data-schedule-id="<?= $schedule['id'] ?>"> <!-- Tambahkan atribut data untuk menyimpan ID jadwal -->
        <th scope="row"><?= $key + 1 ?></th>
        <td><?= $schedule['nama'] ?></td>
        <td><?= $schedule['email'] ?></td>
        <td><?= $schedule['jam'] ?></td>
        ...
        <td><?= $schedule['nama_coach'] ?></td>
        <td><?= $schedule['keterangan_coach'] ?></td>
        <td class="status-col"> <!-- Tambahkan kelas CSS untuk kolom status -->
...
$(document).ready(function() {
    $('.accept-btn').click(function() {
        ...
        var scheduleId = row.data('schedule-id'); // Mengambil ID jadwal dari atribut data
